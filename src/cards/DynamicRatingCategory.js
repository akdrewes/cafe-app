...
export default function DynamicRatingCategory({
  colour,
  fontSize,
  label,
  onToggleIcon,
  ...
  return (
    <RatingCategoryStyle>
      <Label color={colour} fontSize={fontSize}>
        {label}
      </Label>
      <Rating>
        {new Array(5).fill('').map((placeholder, index) => (
          <RatingIcon
            key={index}
            onClick={() => onToggleIcon(index)}
            src={index <= activeIndex ? activeIcon : disabledIcon}
          />
        ))}
      </Rating>
    </RatingCategoryStyle>
